// =============================================================================
// ФАЙЛ: crates/core/database/src/models/messages/model.rs
// НАЗНАЧЕНИЕ: Модель данных сообщения и бизнес-логика его жизненного цикла.
//
// КЛЮЧЕВЫЕ КОНЦЕПТЫ ФАЙЛА:
// - auto_derived_partial! — генерация структуры Message + PartialMessage
// - auto_derived! — генерация связанных enum/struct с стандартными трейтами
// - MessageFlagsValue — работа с битовыми флагами
// - async-методы на структуре — бизнес-логика (отправка, удаление, реакции)
// - EventV1 — публикация событий для WebSocket-клиентов
// =============================================================================
...
// `auto_derived_partial!` — макрос из database/src/lib.rs.
// Генерирует СРАЗУ ДВЕ структуры:
// 1. `Message` — полная структура сообщения (хранится в БД и отправляется клиентам)
// 2. `PartialMessage` — частичная версия, где каждое поле обёрнуто в Option<T>
//    Используется для PATCH-обновлений: клиент присылает только изменённые поля.
auto_derived_partial!(
    /// Сообщение в чате.
    pub struct Message {
        // `#[serde(rename = "_id")]` — MongoDB хранит идентификаторы в поле `_id`,
        // но в нашем Rust-коде мы называем его `id`. Атрибут задаёт маппинг имён.
        /// Уникальный идентификатор сообщения (ULID — лексикографически сортируемый UUID)
        #[serde(rename = "_id")]
        pub id: String,

        // `Option<String>` — поле может отсутствовать (None) или содержать значение (Some).
        // `#[serde(skip_serializing_if = "Option::is_none")]` — если поле None,
        // оно вообще не попадает в JSON. Экономит трафик для необязательных полей.
        /// Уникальное значение от клиента для идемпотентности — предотвращает дублирование
        /// при повторной отправке одного и того же сообщения (при сетевых проблемах).
        #[serde(skip_serializing_if = "Option::is_none")]
        pub nonce: Option<String>,
        /// Идентификатор канала, в котором отправлено сообщение
        pub channel: String,
        /// Идентификатор пользователя или вебхука, отправившего сообщение
        pub author: String,
        /// Данные вебхука, если сообщение отправлено через вебхук (а не пользователем)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub webhook: Option<MessageWebhook>,
        /// Текстовое содержимое сообщения (None если только вложения или системное событие)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub content: Option<String>,
        /// Системное событие (вступление пользователя, переименование канала и т.д.)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub system: Option<SystemMessage>,
        /// Прикреплённые файлы — `Vec<File>` это динамический массив структур File
        #[serde(skip_serializing_if = "Option::is_none")]
        pub attachments: Option<Vec<File>>,
        /// Время последнего редактирования. `Timestamp` из iso8601_timestamp — типобезопасная дата/время.
        #[serde(skip_serializing_if = "Option::is_none")]
        pub edited: Option<Timestamp>,
        /// Встроенные превью (embed'ы) — для ссылок, изображений и т.д.
        #[serde(skip_serializing_if = "Option::is_none")]
        pub embeds: Option<Vec<Embed>>,
        /// Список ID пользователей, упомянутых в сообщении (через @mention)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub mentions: Option<Vec<String>>,
        /// Список ID ролей, упомянутых в сообщении (через @роль)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub role_mentions: Option<Vec<String>>,
        /// Список ID сообщений, на которые отвечает это сообщение
        #[serde(skip_serializing_if = "Option::is_none")]
        pub replies: Option<Vec<String>>,
        // `IndexMap<K, V>` — хэш-карта с сохранением порядка вставки (из крейта indexmap).
        // Ключ — ID эмодзи, значение — `IndexSet<String>` (множество ID пользователей).
        // `skip_serializing_if = "IndexMap::is_empty"` — не сериализовать пустую карту.
        // `default` — при десериализации использовать пустую карту если поле отсутствует.
        /// Реакции: {emoji_id: [user_id, ...]}
        #[serde(skip_serializing_if = "IndexMap::is_empty", default)]
        pub reactions: IndexMap<String, IndexSet<String>>,
        /// Настройки взаимодействия: какие реакции разрешены, ограничения
        #[serde(skip_serializing_if = "Interactions::is_default", default)]
        pub interactions: Interactions,
        /// Маскарад — переопределение имени/аватара отправителя для этого сообщения
        #[serde(skip_serializing_if = "Option::is_none")]
        pub masquerade: Option<Masquerade>,
        /// Закреплено ли сообщение в канале
        #[serde(skip_serializing_if = "crate::if_option_false")]
        pub pinned: Option<bool>,

        // `u32` — беззнаковое 32-битное целое. Флаги хранятся как битовые поля:
        // каждый бит отвечает за отдельный флаг (suppress notifications, mention all и т.д.)
        /// Битовые флаги сообщения (управление уведомлениями, массовые упоминания)
        #[serde(skip_serializing_if = "Option::is_none")]
...
// `auto_derived!` — макрос добавляет Serialize, Deserialize, Debug, Clone, Eq, PartialEq.
// `#[serde(tag = "type")]` — "internally tagged" enum:
// при сериализации каждый вариант получает поле `"type"` с именем варианта.
// Например: {"type": "user_joined", "id": "01HX..."}
// `#[serde(rename = "...")]` — задаёт конкретное строковое значение для поля "type",
// следуя snake_case-конвенции JSON API.
auto_derived!(
    /// Системное событие — автоматическое сообщение от сервера (не от пользователя).
    /// Примеры: вступление в канал, бан, переименование, закрепление.
    #[serde(tag = "type")]
...
// `MessageFlagsValue(pub u32)` — "newtype" паттерн в Rust:
// обёртка вокруг примитивного типа u32 с именем MessageFlagsValue.
// Это создаёт типобезопасность — нельзя случайно перепутать флаги с другими u32.
// `pub u32` — внутреннее поле публично, к нему можно обратиться через `.0`.
pub struct MessageFlagsValue(pub u32);

impl MessageFlagsValue {
    // Проверяет наличие конкретного флага из перечисления MessageFlags.
    // `flag as u32` — приведение enum-варианта к числу (каждый вариант = номер бита).
    pub fn has(&self, flag: MessageFlags) -> bool {
        self.has_value(flag as u32)
    }

    // Проверяет установлен ли конкретный бит по его номеру.
    // `1 << bit` — сдвиг единицы влево на `bit` позиций = маска нужного бита.
    // `self.0 & mask == mask` — побитовое И: если бит установлен, результат равен маске.
    ...
    // `-> &mut Self` — возвращает мутабельную ссылку на себя для цепочки вызовов (builder-паттерн).
    ...
    pub fn set_value(&mut self, bit: u32, toggle: bool) -> &mut Self {
        if toggle {
            // `|=` — побитовое ИЛИ с присвоением: устанавливает бит в 1, не трогая остальные.
            self.0 |= 1 << bit;
        } else {
            // `&=` с инвертированной маской `!(1 << bit)` — сбрасывает бит в 0.
            // `!` — побитовое НЕ (инверсия всех битов маски).
            self.0 &= !(1 << bit);
        }
        self
    }
...
// =============================================================================
// РЕАЛИЗАЦИЯ Default ДЛЯ Message
// =============================================================================

// `#[allow(clippy::derivable_impls)]` — подавляем предупреждение линтера Clippy.
// Clippy хотел бы, чтобы мы использовали `#[derive(Default)]` вместо ручной реализации,
// но здесь это невозможно из-за особенностей макроса auto_derived_partial!.
//
// `impl Default for Message` — реализация стандартного трейта Default.
// Трейт Default позволяет создавать "нулевое" значение типа через `Message::default()`
// или синтаксис `Message { id, ..Default::default() }` (обновление структуры).
...
// `#[allow(clippy::disallowed_methods)]` — разрешаем использование методов,
// которые обычно запрещены правилами clippy.toml (insert_message, update_message и т.д.).
// Это именно то место в коде, где разрешено их использовать напрямую.
//
// Все методы ниже — это ВЫСОКОУРОВНЕВЫЕ операции над сообщениями.
// Они содержат бизнес-логику: валидацию, проверку прав, эмиссию событий.
// Остальной код должен вызывать эти методы, а не обращаться к БД напрямую.
#[allow(clippy::disallowed_methods)]
impl Message {
    /// Создать сообщение из данных API-запроса.
    /// Выполняет: валидацию, обработку упоминаний, создание ID, сохранение в БД.
    // `#[allow(clippy::too_many_arguments)]` — Clippy предупреждает о большом числе
    // аргументов функции (>7). Здесь это оправдано сложностью операции создания сообщения.
